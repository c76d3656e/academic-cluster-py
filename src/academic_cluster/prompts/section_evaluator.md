...
- **禁止括号化引用说明**：如"（文献[24][28]已证实...）"、"（[27][29]）"、"([x])"、"([1][2])"。应要求改为正文句法，如"已有研究[24,28]证实..."或"该局限已有研究讨论[27,29]"。
- **禁止"文献[N]"用法**：任何形式的"文献[N]"都不允许——包括作主语（"文献[25]指出..."）、所有格修饰语（"文献[25]的研究显示..."）、介词宾语（"根据文献[25]..."）。应改为：事实/方法作主语+[N]放句末，或用"作者等[N]"引导，或用"有研究[N]"引导。
...
以下展示了不同质量水平的评估输出，请参照此标准进行评分：

### 高质量章节（score=85, needs_revision=false）

```json
{{
  "score": 85,
  "dimensions": {{
    "coverage": {{"score": 88, "comment": "覆盖了段落规划中的全部5个核心论点，key_papers引用完整。但对mechanism类型段落的机理分析深度不足，仅描述了现象而未深入解释驱动因素。"}},
    "logic": {{"score": 82, "comment": "段落间逻辑链完整，按'问题→方案→对比→展望'组织。第3段到第4段存在轻微跳跃，缺少从实验结果到未来展望的过渡性分析。"}},
    "citations": {{"score": 90, "comment": "引用密度合理（段均3.2处），格式全部为[N]编号。第2段有一处引用[7]的论点与原文不完全匹配，建议核实。"}},
    "transitions": {{"score": 80, "comment": "与前序章节的衔接自然，但结尾段未为后序章节做铺垫，缺少过渡性语句。"}},
    "style": {{"score": 82, "comment": "无禁用表达，句式变化较好。但第1段连续3句以'该'字开头，建议调整。"}}
  }},
  "revision_instructions": "在第3段末尾添加对mechanism的深入分析；第4段开头增加与第3段的过渡语句；结尾段添加为后序章节的铺垫；调整第1段的句式多样性。",
  "needs_revision": false
}}
```

### 低质量章节（score=62, needs_revision=true）

```json
{{
  "score": 62,
  "dimensions": {{
    "coverage": {{"score": 55, "comment": "仅覆盖了5个核心论点中的3个，遗漏了comparison和mechanism类型段落的关键内容。key_papers中[3][5][7]未被引用或讨论。"}},
    "logic": {{"score": 60, "comment": "存在明显的文献堆叠模式——第2段连续介绍了4篇论文而无综合分析。段落之间缺乏逻辑递进，呈现并列罗列而非分析推进。"}},
    "citations": {{"score": 70, "comment": "引用格式基本正确，但第3段有两处'文献[12]指出...'的违规用法（文献[N]作主语）。引用密度偏低，第1段仅有1处引用。"}},
    "transitions": {{"score": 65, "comment": "段落间缺少连接词，第2段到第3段之间存在明显的内容断裂。与前序章节无过渡。"}},
    "style": {{"score": 60, "comment": "检测到'综上所述'1处、'从技术演进角度'1处。第1段和第3段均以'在...'开头，违反段落多样性要求。"}}
  }},
  "revision_instructions": "第2段：将逐篇介绍改为对比分析，按方法类别组织而非按论文编号。补充[3][5][7]的引用和讨论。第1段和第3段：修改开头，避免重复句式。删除'综上所述'和'从技术演进角度'。在段落间添加过渡语句。",
  "needs_revision": true
}}
```
